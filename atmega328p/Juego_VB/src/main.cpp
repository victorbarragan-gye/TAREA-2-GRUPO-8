...
int main(void) {
  ...
  while (1) {
    ...
    while (vivo) {
      ...
      if (salto_simple && salto == 0) { salto = 1; altura = 0; }
      if (salto_doble  && salto == 0) { salto = 2; altura = 0; }
      if (bajar && (salto == 1 || salto == 2)) {
        jugador_y = 4; salto = 0; altura = 0;
      }

      // Física
      if (salto == 1) {
        if (altura < 3) jugador_y--;
        else if (altura < 6) jugador_y++;
        else salto = 0;
        altura++;
      }
      else if (salto == 2) {
        if (altura < 5) jugador_y--;
        else if (altura < 10) jugador_y++;
        else salto = 0;
        altura++;
      }

      if (jugador_y < 0) jugador_y = 0;
      if (jugador_y > 4) jugador_y = 4;

      unsigned char *sprite;
      if (salto == 1) sprite = JUGADOR_SALTO;
      else if (salto == 2) sprite = JUGADOR_SALTO2;
      else sprite = JUGADOR_SUELO;

      dibujar_frame(sprite, mapa_index);

      // Colisión
      for (int f=0; f<8; f++) {
        if (MAPA[mapa_index+f] & sprite[f]) vivo = 0;
      }
